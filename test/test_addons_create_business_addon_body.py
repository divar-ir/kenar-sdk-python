# coding: utf-8

"""
    API کنار

    No description provided (generated by Openapi Generator https://github.com/openapitools/openapi-generator)

    The version of the OpenAPI document: 1.0
    Generated by OpenAPI Generator (https://openapi-generator.tech)

    Do not edit the class manually.
"""  # noqa: E501


import unittest

from kenar_api_client.models.addons_create_business_addon_body import AddonsCreateBusinessAddonBody

class TestAddonsCreateBusinessAddonBody(unittest.TestCase):
    """AddonsCreateBusinessAddonBody unit test stubs"""

    def setUp(self):
        pass

    def tearDown(self):
        pass

    def make_instance(self, include_optional) -> AddonsCreateBusinessAddonBody:
        """Test AddonsCreateBusinessAddonBody
            include_optional is a boolean, when False only required
            params are included, when True both required and
            optional params are included """
        # uncomment below to create an instance of `AddonsCreateBusinessAddonBody`
        """
        model = AddonsCreateBusinessAddonBody()
        if include_optional:
            return AddonsCreateBusinessAddonBody(
                widgets = [
                    kenar_api_client.models.addons_widget.addonsWidget(
                        button_bar = kenar_api_client.models.addons_button_bar.addonsButtonBar(
                            action = kenar_api_client.models.عملیات.عملیات(
                                get_dynamic_action = kenar_api_client.models.دریافت_action_پویا.دریافت Action پویا(
                                    data = {"your-internal-id":"something"}, ), 
                                open_direct_link = 'https://your-site.com/some-path', 
                                open_post_manage_page = kenar_api_client.models.باز_کردن_صفحه_مدیریت_آگهی.باز کردن صفحه مدیریت آگهی(
                                    post_token = 'AJIEWcw', ), 
                                open_post_page = kenar_api_client.models.باز_کردن_صفحه_آگهی.باز کردن صفحه آگهی(
                                    post_token = 'AJIEWcw', ), 
                                open_server_link = kenar_api_client.models.باز_کردن_لینک_سرور.باز کردن لینک سرور(
                                    data = {"your-internal-id":"something"}, ), ), 
                            title = '', ), 
                        description_row = kenar_api_client.models.addons_description_row.addonsDescriptionRow(
                            expandable = True, 
                            has_divider = True, 
                            text = '', ), 
                        evaluation_row = kenar_api_client.models.addons_evaluation_row.addonsEvaluationRow(
                            icon_name = 'KEYBOARD_ARROW_RIGHT', 
                            indicator_percentage = 56, 
                            indicator_text = '', 
                            left = kenar_api_client.models.addons_evaluation_row_section.addonsEvaluationRowSection(
                                section_color = 'WARNING_SECONDARY', 
                                text = '', ), 
                            middle = kenar_api_client.models.addons_evaluation_row_section.addonsEvaluationRowSection(
                                text = '', ), 
                            right = , ), 
                        event_row = kenar_api_client.models.addons_event_row.addonsEventRow(
                            has_divider = True, 
                            image_id = '', 
                            label = '', 
                            subtitle = '', 
                            title = '', ), 
                        group_info_row = kenar_api_client.models.addons_group_info_row.addonsGroupInfoRow(
                            has_divider = True, 
                            items = [
                                kenar_api_client.models.addons_group_info_row_group_info_item.addonsGroupInfoRowGroupInfoItem(
                                    title = '', 
                                    value = '', )
                                ], ), 
                        image_carousel_row = kenar_api_client.models.addons_image_carousel_row.addonsImageCarouselRow(
                            has_divider = True, ), 
                        score_row = kenar_api_client.models.addons_score_row.addonsScoreRow(
                            descriptive_score = '', 
                            has_divider = True, 
                            percentage_score = 56, 
                            score_color = 'WARNING_SECONDARY', 
                            title = '', ), 
                        selector_row = kenar_api_client.models.addons_selector_row.addonsSelectorRow(
                            has_divider = True, 
                            image_id = '', 
                            title = '', ), 
                        semantic_paths = {
                            'key' : ''
                            }, 
                        subtitle_row = kenar_api_client.models.addons_subtitle_row.addonsSubtitleRow(
                            has_divider = True, 
                            text = '', ), 
                        title_row = kenar_api_client.models.addons_title_row.addonsTitleRow(
                            has_divider = True, 
                            text = '', ), )
                    ]
            )
        else:
            return AddonsCreateBusinessAddonBody(
        )
        """

    def testAddonsCreateBusinessAddonBody(self):
        """Test AddonsCreateBusinessAddonBody"""
        # inst_req_only = self.make_instance(include_optional=False)
        # inst_req_and_optional = self.make_instance(include_optional=True)

if __name__ == '__main__':
    unittest.main()
